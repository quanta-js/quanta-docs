...
export function CopyButton() {
    ...
    const handleCopyPage = async () => {
        ...
        try {
            ...
            toast({
                ...
                title: 'Copied!',
                description: 'Page content copied as Markdown.',
            });
        } catch (error) {
            console.log(error)
            toast({
                className: 'bg-black bg-opacity-50 backdrop-blur-md',
                title: 'Error',
                description: 'Failed to copy page content.',
                variant: 'destructive',
            });
        } finally {
            setIsCopying(false);
    ...
    const handleViewMarkdown = () => {
        const apiUrl = `/api/docs/raw/${slug}`;
        window.open(apiUrl, '_blank'); // Open API route in a new tab
    };

    const getLLMPrompt = () => {
        const pageOrigin = window.location.origin;
        const link = `${pageOrigin}/api/docs/raw/${slug}`;
        return `Please read and analyze the following documentation page: ${link}, and answer questions about the content`;
    };

    const handleOpenChatGPT = async () => {
        await navigator.clipboard.writeText(getLLMPrompt());
        window.open('https://chatgpt.com/?q=' + getLLMPrompt(), '_blank');
        toast({
            className: 'bg-black bg-opacity-50 backdrop-blur-md',
            title: 'Prompt copied!',
            description: 'Paste the prompt in ChatGPT to ask about this page.',
        });
    };
    ...
    const handleOpenClaude = async () => {
        ...
        toast({
            className: 'bg-black bg-opacity-50 backdrop-blur-md',
            title: 'Prompt copied!',
            description: 'Paste the prompt in Claude to ask about this page.',
        });
    };

    return (
        <div className="flex items-center text-base">
            <Button variant="outline" disabled={isCopying} onClick={handleCopyPage}>
                <Clipboard className="w-4 h-4 mr-2" />
                {isCopying ? 'Copying...' : 'Copy'}
            </Button>
            <DropdownMenu onOpenChange={(open) => setOpen(open)}>
                <DropdownMenuTrigger asChild >
                    <Button variant="outline" disabled={isCopying} className="border">
                        {
