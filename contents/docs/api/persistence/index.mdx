...
> **Note:** Unlike `LocalStorageAdapter`, `SessionStorageAdapter` does not support cross-tab synchronization because `storage` events do not fire for `sessionStorage` changes within the same tab.
...
The `LocalStorageAdapter` supports cross-tab synchronization automatically via `storage` events. When data changes in one tab, other tabs are automatically updated:
...
> **Note:** Cross-tab sync is only available with `LocalStorageAdapter`. `SessionStorageAdapter` does not support cross-tab sync, and `IndexedDBAdapter` does not have a built-in change notification mechanism.
