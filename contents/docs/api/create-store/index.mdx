...
- **Name Collisions**: Throws if an action name conflicts with a state property or getter name. Getters that shadow state properties are allowed (the getter takes priority on the flattened store)
- **Destroyed Store**: Using a store after `$destroy()` may result in errors

## Batch Effects

When making multiple state changes that should be treated as a single update, use `batchEffects`:
...
// Effects run only once after the batch completes
```
