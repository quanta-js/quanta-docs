...
- `options` (optional): Configuration options:
  - `deep`: Whether to perform deep watching (defaults to `false`). When `true`, recursively tracks all nested properties for changes.
  - `immediate`: Whether to invoke the callback once immediately upon setup (defaults to `false`).
  - `equals`: Decides whether a re-run produced the same value; the callback fires only when it returns `false`. Defaults to `Object.is`. Ignored with `deep`.
...
// Watch a single value (the callback runs on the next change)
...
By default the callback runs only when the value changes. Pass `immediate: true` to also run it once when the watcher is set up:

```javascript
const state = reactive({ count: 0 });

// Only runs when count changes (default)
watch(() => state.count, (value) => {
  console.log(value);
});

// Also runs immediately with the current value
watch(() => state.count, (value) => {
  console.log(value);
}, { immediate: true });
```
...
For deep watching of nested objects, use the `deep` option. This recursively tracks all nested properties using reactive dependency tracking:
...
// Deep watch - detects nested property changes reactively
...
**Note:** Deep watching recursively accesses all nested properties to register reactive dependencies. For very large or deeply nested objects, prefer watching specific properties for better performance.

### Custom Equality

The callback fires when the source returns a value that is not `Object.is`-equal to the previous one. A source that builds a new object on each run is never equal, so pass `shallow` to compare it one level deep:

```javascript
import { reactive, shallow, watch } from '@quantajs/core';

const form = reactive({ errors: [], touched: [] });

// Re-runs whenever either list changes, but fires only when
// `valid` or `dirty` actually flips.
watch(
  () => ({ valid: form.errors.length === 0, dirty: form.touched.length > 0 }),
  (status) => updateSubmitButton(status),
  { equals: shallow },
);
```

The opposite also works: `equals: () => false` fires on every change the source tracks, including an in-place mutation of an object it returns by identity.
