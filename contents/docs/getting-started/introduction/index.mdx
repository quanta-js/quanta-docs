...
- **Any Framework**: Official bindings for React, Vue, Svelte and Lit, an Astro integration that shares one store across islands, and a core that runs in any JavaScript environment.
...
### @quantajs/vue
Vue composables with the same API: `useQuanta`, `useQuantaValue`, `useQuantaActions` and `useLocalStore`, plus the `createQuanta` plugin for server rendering.

```javascript
import { useQuantaValue, createQuanta } from '@quantajs/vue';
```

### @quantajs/svelte
The same functions, returning standard Svelte stores for `$` subscriptions in Svelte 4, Svelte 5 and SvelteKit.

```javascript
import { useQuantaValue, setQuantaContainer } from '@quantajs/svelte';
```

### @quantajs/lit
Reactive controllers for web components: `QuantaValueController`, `QuantaController`, `QuantaActionsController` and `QuantaLocalController`, with containers scoped through the web components context protocol.

```javascript
import { QuantaValueController, provideQuantaContainer } from '@quantajs/lit';
```

### @quantajs/astro
An Astro integration: a container per request, server state that reaches the islands, and one store shared by React, Vue and Svelte islands.
...
Upgrading from 2.x? [Migrating to 3.0](/docs/getting-started/migration) covers everything that changed.
