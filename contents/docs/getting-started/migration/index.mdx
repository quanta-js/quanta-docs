...
All packages now share one version number, so `@quantajs/core`, `@quantajs/react`, `@quantajs/devtools`, `@quantajs/vue` and `@quantajs/svelte` are released together as 3.0.0. Update them together:
