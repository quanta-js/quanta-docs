...
3.0 removes APIs that were deprecated or never meant to be public, types persistence end to end, and fixes how development builds are detected. It also adds official [Vue](/docs/integration/vue-integration), [Svelte](/docs/integration/svelte-integration) and [Lit](/docs/integration/lit-integration) bindings, and an [Astro integration](/docs/integration/astro-integration). Stores themselves are unchanged: a `defineStore` definition from 2.x works as it is.
...
- **Lit bindings**: [`@quantajs/lit`](/docs/integration/lit-integration), the same four as reactive controllers for web components.
- **Astro integration**: [`@quantajs/astro`](/docs/integration/astro-integration) gives each request its own container, carries server state to the islands, and lets React, Vue and Svelte islands share one store.
- **`setDefaultContainerResolver`**, for any server that renders components without passing a container: resolve the default container per request, for example from `AsyncLocalStorage`. See [Containers](/docs/api/containers#a-request-scoped-default-container).
- **`CookieAdapter`**, to persist small state, such as a theme, in a cookie the server also receives. See [Persistence](/docs/api/persistence#cookieadapter).
