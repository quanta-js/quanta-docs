...
## Async Actions Come With a Lifecycle

Before reaching for a watcher or a hand-rolled flag: every action already
carries `pending`, `error` and `abort()`, and `this.$signal` inside the action
is an `AbortSignal` scoped to that call.
...
users.load.pending;   // boolean, reactive
users.load.error;     // Error | null — cleared at the start of every call
users.load.abort();   // cancels every in-flight call
```

That covers the most common side effect — "fetch something and show a spinner"
— without a `isLoading` field, an `error` field, or a `finally` you can forget
to write. The rest of this page is for the effects it does *not* cover:
reacting to a state change, syncing to an external system, timers, sockets.

See [Async Actions](/docs/guides/async-actions) for the full surface.
...
function UserProfile() {
  const user = useQuantaValue(useUserStore, (s) => s.user);
  // The action's own lifecycle — no `isLoading` field in state.
  const isLoading = useQuantaValue(useUserStore, (s) => s.load.pending);
