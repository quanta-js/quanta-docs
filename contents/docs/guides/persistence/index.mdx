...
QuantaJS provides four storage adapters for different use cases:
...
For a small piece of state the server should also see, such as a theme or locale. The cookie travels with every request, so persist only what the server needs:

```javascript
import { CookieAdapter } from '@quantajs/core';

const store = createStore('prefs', {
  state: () => ({ theme: 'light', recentSearches: [] }),
  persist: {
...
A cookie holds at most 4096 bytes. A write that would exceed that throws, and the store reports it through `onError` while the previous cookie stays in place.
...
Handle complex data types that can't be serialized with JSON. `serialize` receives the whole stored envelope, `{ data, version, timestamp, storeName }`, not only the state, and `deserialize` must return that shape. A JSON replacer and reviver pair, as here, handles that naturally.
...
const store = createStore('validated', {
  ...
  persist: {
    ...
    onError: (error, operation) => {
      // Stored data that fails the validator is not loaded, so the store
      // keeps its defaults. Remove it so the next visit starts clean.
      if (operation === 'read') store.$persist?.clear();
    }
  }
});
```
...
A migration is a plain function from the data as stored under the previous version to the new shape. The usual changes take a line or two each:
...
const store = createStore('patterns', {
  ...
  persist: {
    ...
    migrations: {
      // 1 → 2: add a property
      2: (data) => ({ ...data, settings: { language: 'en', ...data.settings } }),
      // 2 → 3: rename a property
      3: ({ user, ...rest }) => ({ ...rest, currentUser: user }),
      // 3 → 4: transform a property
      4: (data) => ({
        ...data,
        settings: {
          ...data.settings,
          theme: data.settings.theme === 'auto' ? 'light' : data.settings.theme
        }
      })
    }
  }
...
`$hydrated` is a promise, so there is nothing to poll. It resolves immediately
when the store has no persistence configured, which means calling code does not
have to branch on whether persistence is on. For a synchronous check,
`store.$persist?.isRehydrated()` is still available.

## Cross-tab Synchronization

### Automatic Synchronization

The `LocalStorageAdapter` automatically synchronizes data across browser tabs via `storage` events:
...
> **Note:** Cross-tab synchronization is only supported with `LocalStorageAdapter`. `SessionStorageAdapter` does not support it because `storage` events do not fire for same-tab `sessionStorage` changes. `IndexedDBAdapter` does not have a built-in change notification mechanism.
