...
Enable the bridge at your app's entry point. Stores created before it is
enabled are picked up too, so it also works when `<QuantaDevTools>` turns it on
...
Paths are matched against the state and getter values the panel shows, keys
nested inside a changed value, and action arguments that are objects carrying a
matching key. Matches are replaced with `[redacted]`.
...
1. **The store is in a different container.** A store in a per-request or
   `useLocalStore` container has its own lifetime and may already be disposed;
   destroyed stores are removed from the panel.
2. **Duplicate names.** Two stores sharing a name in one container are one
