...
For data that needs to persist across sessions, use QuantaJS's built-in persistence:
...
export const cartStore = createStore('cart', {
  ...
  persist: {
    adapter: new LocalStorageAdapter('cart-store'),
    debounceMs: 300,
  },
});
// State is automatically saved and restored — no manual localStorage code needed!
```
...
if (process.env.NODE_ENV === 'development') {
  // Log store changes
  userStore.subscribe(() => {
    console.log('User store changed:', userStore.state);
  });
}
```
...
- [React Integration](/docs/integration/react-integration) - Advanced React patterns
- [Tips and Examples](/docs/best-practices/tips-and-examples) - Best practices and patterns
- [Examples](/docs/best-practices/examples) - Complete working examples
