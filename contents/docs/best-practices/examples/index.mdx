...
```javascript
import { defineStore, watch } from '@quantajs/core';
...
// A definition is a blueprint; resolve it to get the instance.
...
function TodoItem({ todo }) {
  const store = useQuantaActions(useTodoStore);
...
// Todo List Component
function TodoList() {
  const todos = useQuantaValue(useTodoStore, store => store.filteredTodos);
...
// Filter Component
function TodoFilter() {
  const filter = useQuantaValue(useTodoStore, store => store.filter);
  const completedCount = useQuantaValue(useTodoStore, store => store.completedCount);
  const pendingCount = useQuantaValue(useTodoStore, store => store.pendingCount);
  const totalCount = useQuantaValue(useTodoStore, store => store.todos.length);
...
function LoginForm() {
  ...
  const store = useQuantaActions(useUserStore);
...
// Stats Component
function Stats() {
  const stats = useQuantaValue(useDashboardStore, store => store.stats);
  const isLoading = useQuantaValue(useDashboardStore, store => store.isLoading);
...
function RecentActivity() {
  const activity = useQuantaValue(useDashboardStore, store => store.recentActivity);
...
// Dashboard Component
function Dashboard() {
  const user = useQuantaValue(useUserStore, store => store.user);
  const store = useQuantaActions(useUserStore);
  // Resolved through the hook so it comes from the provider's container —
  // calling useDashboardStore() directly would read the ambient one.
  const dashboard = useQuantaActions(useDashboardStore);
...
// Main App Component
function App() {
  const isAuthenticated = useQuantaValue(useUserStore, store => store.isAuthenticated);
...
- [Tips and Examples](/docs/best-practices/tips-and-examples) - Practical tips and patterns
