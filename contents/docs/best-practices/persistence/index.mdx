...
// Gate on restoration before using persisted data
await store.$hydrated;
```

`$hydrated` is a promise rather than a flag to poll, and it resolves
immediately when a store has no persistence configured — so code that awaits it
does not have to know whether persistence is on. In React, await it once during
app startup rather than branching in every component:

```tsx
// app entry
await useUserStore().$hydrated;
...
Write one plain function per version, each doing one thing, so every step is easy to test on its own:
