...
export function Header() {
  const user = useQuantaSelector(userStore, store => store.user);
  const cartItems = useQuantaSelector(cartStore, store => store.totalItems);
  const isCartOpen = useQuantaSelector(cartStore, store => store.isOpen);
...
export function CartSummary() {
  ...
  const totalPrice = useQuantaSelector(cartStore, store => store.totalPrice);
