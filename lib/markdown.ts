...
function rehypeEnhancedCodeBlocks() {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return (tree: any) => {
        visit(tree, "element", (node) => {
            if (
                ...
                // Find the class that contains the language and metadata
                ...
                if (languageClass) {
                    ...
                    const fullClass = languageClass.replace("language-", "");
                    ...
                    for (let i = 1; i < parts.length; i++) {
                        ...
                        if (part === "showLineNumbers") {
                            ...
                            // Parse highlight lines like {3-4} or {2,3-5}
                            const lineSpec = part.replace(/[{}]/g, "");
                            const ranges = lineSpec.split(",");
                            ...
                            // First non-special part is the filename
                            filename = part;
                        }
                    }

                    // Update the code node properties
                    if (filename) {
                        codeNode.properties["data-filename"] = filename;
                    }

                    if (showLineNumbers) {
                        codeNode.properties["data-line-numbers"] = "true";
                    }

                    if (highlightLines.length > 0) {
                        codeNode.properties["data-highlight-lines"] =
                            highlightLines.join(",");
                    }

                    // Update the language class to only include the language
                    const newClassNames = classNames.filter(
                        (cls) => !cls?.startsWith("language-")
                    );
                    newClassNames.push(`language-${language}`);
                    codeNode.properties.className = newClassNames;
                }
            }
        });
    };
